...
const testimonialsData = [ { name: 'Jessica Smith', title: 'COO, Innovate Inc.', text: '"A complete game-changer. The ClickUp integration they built saves our team at least 10 hours a week."', avatar: 'JS' }, { name: 'Marcus Aurelius', title: 'Marketing Head, DataDriven Co.', text: '"The custom reporting system is phenomenal. Their understanding of Make.com was impressive."', avatar: 'MA' }, { name: 'Emily Carter', title: 'Founder, Creative Solutions', text: '"They used Zapier to connect all our disparate tools into one cohesive system. I highly recommend them."', avatar: 'EC' }, { name: 'Rachel Brown', title: 'Director, Onboardify Agency', text: '"Our client onboarding is now flawless and fully automated. The attention to detail was fantastic."', avatar: 'RB' }, { name: 'David Kim', title: 'VP of Sales, GrowthRocket', text: '"I was skeptical about automating our sales process. Automation Live proved me wrong. Highly effective."', avatar: 'DK' } ];
...
const Contact = () => {
    ...
    const handleSubmit = (e) => { e.preventDefault(); setStatus('Thank you! Your message has been sent.'); e.target.reset(); setTimeout(() => setStatus(''), 5000); };
    return (<section id="contact" className="py-20 bg-white"><div className="container mx-auto px-6"><div className="text-center mb-12"><h2 className="text-3xl md:text-4xl font-bold">Let's Build Your Automated Future</h2><p className="text-gray-600 mt-2 max-w-2xl mx-auto">Have a question or want to start a project? Send us a message.</p></div><div className="max-w-2xl mx-auto bg-gray-50 p-8 sm:p-10 rounded-xl shadow-lg border"><form onSubmit={handleSubmit}><div className="mb-6"><label htmlFor="name" className="block font-medium mb-2">Full Name</label><input type="text" id="name" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" required /></div><div className="mb-6"><label htmlFor="email" className="block font-medium mb-2">Email Address</label><input type="email" id="email" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" required /></div><div className="mb-6"><label htmlFor="message" className="block font-medium mb-2">Message</label><textarea id="message" rows="5" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500" required></textarea></div><button type="submit" className="w-full bg-blue-600 text-white font-semibold p-4 rounded-lg hover:bg-blue-700 cta-button">Send Message</button></form>{status && <div className="mt-4 text-center text-green-600 font-medium">{status}</div>}</div></div></section>);
};
...
const BookingModal = ({ isOpen, onClose }) => {
    ...
    const resetForm = useCallback(() => { setSelectedDate(null); setSelectedTime(null); setDetails({ name: '', email: '' }); setStep('booking'); }, []);
    useEffect(() => { if (!isOpen) { const timer = setTimeout(resetForm, 300); return () => clearTimeout(timer); } }, [isOpen, resetForm]);
    const handleConfirm = () => { const dateStr = selectedDate.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}); const subject = `New Consultation: ${details.name} on ${dateStr}`; const body = `Details:\nName: ${details.name}\nEmail: ${details.email}\nDate: ${dateStr}\nTime: ${selectedTime}`; window.location.href = `mailto:contact@automationlive.dev?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`; setStep('confirmation'); };
    const handleAddToCalendar = () => { const [time, modifier] = selectedTime.split(' '); let [hours, minutes] = time.split(':'); if (hours === '12') hours = '00'; if (modifier === 'PM') hours = parseInt(hours, 10) + 12; const start = new Date(selectedDate); start.setHours(hours, minutes); const end = new Date(start.getTime() + 60*60*1000); const toICS = (d) => d.toISOString().replace(/[-:]/g, '').split('.')[0]+'Z'; const ics = `BEGIN:VCALENDAR\nVERSION:2.0\nBEGIN:VEVENT\nSUMMARY:Consultation: AutomationLive with ${details.name}\nDTSTART:${toICS(start)}\nDTEND:${toICS(end)}\nLOCATION:Online\nEND:VEVENT\nEND:VCALENDAR`; const blob = new Blob([ics], {type:'text/calendar'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'appointment.ics'; a.click(); URL.revokeObjectURL(url); };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl transform scale-100" onClick={e => e.stopPropagation()}>
                ...
                {step === 'booking' ? (<>
                    ...
                    <div className="text-center p-8"><div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><Icon name="check" className="text-green-600 w-10 h-10" /></div><h3 className="text-2xl font-bold">Appointment Request Sent!</h3><p className="text-gray-600 mt-2">Please check your email client to send the request. You can also add this event to your calendar.</p><button onClick={handleAddToCalendar} className="mt-6 w-full bg-blue-600 text-white font-semibold py-3 rounded-lg cta-button flex items-center justify-center gap-2"><Icon name="calendar-plus" /> Add to Your Calendar</button></div>
                )}
            </div>
        </div>
    );
